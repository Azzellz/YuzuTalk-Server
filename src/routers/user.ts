...
router.get("/user", async (req: Request, res: Response) => {
    ...
    try {
        //获取根据分页过滤的用户信息
        const user = await db.user.findById(id, shadowFields).populate([
            {
                path: "follows",
                select: SelectUser,
            },
...
router.get("/users", async (req: Request, res: Response) => {
    ...
    //生成正则:忽略大小写
    const filter = new RegExp(userNameKeyword, "i");
    try {
        const data = await db.user
            .find({ user_name: filter })
            .select(SelectUser);
        res.status(200).send({
            msg: "查找成功",
            data,
...
//关注用户
router.put("/user/follow", async (req: Request, res: Response) => {
...
//取消关注
router.put("/user/unFollow", async (req: Request, res: Response) => {
    const { user_id, follow_id } = req.body;
    try {
        const currentUser = await db.user.findById(user_id);
        if (!currentUser) throw "当前用户不存在";
...
//是否关注用户
router.get("/user/isFollow", async (req: Request, res: Response) => {
